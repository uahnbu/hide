...

{
  ...
  let vowels = ["đ","á","ư","à","ạ","ô","â","ế","ê","ố","ả","ơ","ó","ộ","ấ","ệ","í","ầ","ậ","ờ","ắ","ă","ồ","ớ","ì","ị","ọ","ú","ề","iệ","ụ","ợ","iế","ò","iê","ứ","ổ","ù","ươ","é","ự","ườ","ướ","ủ","ặ","ã","ai","ể","ử","ao","ỏ","ầu","ại","è","oa","ĩ","ái","ở","ua","iề","ừ","áo","ượ","ẩ","ữ","ối","uố","ia","oá","oà","ỉ","ũ","âu","ào","ài","ải","ôi","ội","ằ","ẻ","ẹ","oạ","ói","iể","êu","uâ","ười","ễ","ỗ","ỡ","uộ","uô","ửa","uyê","ồi","ý","ẫ","ạo","ẳ","au","ảo","ưa","ấu","ều","iêu","ĩa","uyế","ếu","eo","uậ","uyề","ệu","iệu","ậu","èo","iều","ỷ","uyệ","õ","uồ","uấ","ổi","uả","ưở","iễ","iếu","ãi","uá","ểu","éo","oả","oi","àu","ẽ","ừa","iểu","oại","ịa","ỳ","ưỡ","ui","áu","ữa","uầ","ùi","ão","ỏi","úi","uế","ẩu","uyể","ựa","òi","ỗi","ứa","oài","uê","ỵ","ẫu","ẹo","ũi","uổ","ưới","ẻo","ía","uẩ","ỹ","oe","oai","ụi","ủi","uạ","oé","ươi","ẵ","ễu","ìa","ỉa","oái","ọi","ưỡi","iễu","oắ","uệ","oă","oè","oẹ","oã","uà","uỗ","ẽo","oặ","õi","oẻ","ảu","uya","oằ","ưởi","uề","uẫ","oải","uyễ","ạu","ượi","oẳ","uã","uỷu","uể","oẵ","oãi","ãu","oẽ","uễ"];
  let letters = 'abcdeghiklmnopqrstuvxy';
  let combs = 'aaabacadaeagahakalapaqarasatavaxeaebecedeeegeheiekelepeqereseteuevexeyibicidigihikilipiqirisitivixobocodogohokolomopoqorosotouovoxubucuduguhukuluoupuqurusutuuuvuxybycydygyhykylymypyqyrysytyvyxyy'.match(/../g);
  ...
  for (let k = 0; k < 10; k++) {
    ...
    let leftovers = vowels.slice(exems.length).filter(c => c.length === 1);
    for (let i = exems.length - 1, k = leftovers.length; i >= 0 && k > 0; i--) {
      exems[i].length > 1 && (exems.splice(i, 1), k--);
    }
    exems.push(...leftovers);
    let excerpt = data;
    sortLen(exems, combs);
    exems.forEach((c, i) => {
...

{
  ...
  exems.push(...leftovers);
  sortLen(exems, combs);
  console.log(JSON.stringify(exems.concat(' ')));
  console.log(combs.slice(0, exems.length).join(''));
...
function hidevn(ctx, msg, key = 'a', pad = 20) {
  var cs = 'abcdeghiklmnopqrstuvxy{';
  var vi = ["iế","ai","iê","ao","ầu","iệ","ườ","ướ","ươ","ừ","ự","ỏ","ở","ĩ","è","ủ","ã","ặ","ử","ể","ỷ","ẳ","õ","ý","ẫ","ỹ","ỵ","ẵ","ẽ","ỳ","ũ","ỉ","ằ","ẩ","ữ","ỗ","ễ","ỡ","ẻ","ẹ","ó","ơ","ộ","ố","ả","í","ầ","ấ","ệ","à","ư","ạ","đ","á","ế","ê","ô","â","ợ","ụ","ò","ú","ề","ù","é","ứ","ổ","ă","ắ","ồ","ậ","ờ","ị","ọ","ớ","ì"," "];
  var en = 'yholaeuperipeuivibelorabikubosapusycygidepovukudakebodaqyraxeeavuvixopiqobesadymucotugohagociraaehahicigihitutacuhaturomegecalulasyteaogeteiuuilisouaruo'.match(/../g).concat`{`;
  var t = msg.replace(new RegExp(`${vi.join('|')}`,'g'), c => en[vi.indexOf(c)]);
  var d = '⁫⁪⁤⁬⁡‬⁪⁡⁣‬⁭‪⁭‪⁮⁣⁯⁯⁢⁢⁬⁫‬⁮⁤', p = '‬‭‬‭‭‬‬‬‬‬‭‭‬‬‬‭‭‬‭‬‬‭‬‭‭', a = 'charCodeAt';
  console.log(t);
  return ctx.slice(0, ctx.length / 2) + '‬'.repeat(pad) + t.split``.map((c,i,s) => (
    s = cs[(cs.indexOf(c) + cs.indexOf(key[i % key.length])) % 23], s == '{' ? '‬' : ((i = p[s[a](0) - 97]) == '‬' ? '' : i) + d[s[a](0) - 97]
  )).join`` + '‬'.repeat(pad) + ctx.slice(ctx.length / 2);
}
...
var zero = {
  b:"abcdeghiklmnopqrstuvxy{",v:"iếaiiêaoầuiệườướươ".match(/../g).concat("ừựỏởĩèủãặửểỷẳõýẫỹỵẵẽỳũỉằẩữỗễỡẻẹóơộốảíầấệàưạđáếêôâợụòúềùéứổăắồậờịọớì ".split``),e:"yholaeuperipeuivibelorabikubosapusycygidepovukudakebodaqyraxeeavuvixopiqobesadymucotugohagociraaehahicigihitutacuhaturomegecalulasyteaogeteiuuilisouaruo{".match(/..?/g),
  hide:function(ctx, msg, key = "a", pad = 20){var g=this,f="indexOf",j="join",x="slice",l="length",r="repeat",t=msg.replace(RegExp(g.v[j]`|`,"g"),c=>g.e[g.v[f](c)]),d="⁫⁪⁤⁬⁡‬⁪⁡⁣‬⁭‪⁭‪⁮⁣⁯⁯⁢⁢⁬⁫‬⁮⁤",p="‬‭‬‭‭‬‬‬‬‬‭‭‬‬‬‭‭‬‭‬‬‭‬‭‭",a="charCodeAt";return ctx[x](0,ctx[l]/2)+"‬"[r](pad)+t.split``.map((c,i,s)=>(s=g.b[(g.b[f](c)+g.b[f](key[i%key[l]]))%23],s=="{"?"‬":((i=p[s[a](0)-97])=="‬"?"":i)+d[s[a](0)-97]))[j]``+"‬"[r](pad)+ctx[x](ctx[l]/2)},
  show:function(enc, key = "a"){var g=this,f="indexOf",w="replace",s="split",j="join",a=["nhticgaumor","lespybvdkxq"],d="‪⁡⁢⁣⁤⁪⁫⁬⁭⁮⁯",r=0;return enc.match(/[‪⁡⁢⁣⁤⁪⁫⁬⁭⁮⁯‬‭]+/)[0][w](/^‬+|‬+$/g,"")[s]``.map(c=>c=="‬"?"{":c=="‭"?(r=1,""):(c=a[r][d[f](c)],r=0,c))[j]``[s]``.map((c,i)=>g.b[(g.b[f](c)-g.b[f](key[i%key.length])+23)%23])[j]``[w](RegExp(g.e[j]`|`,"g"),c=>g.v[g.e[f](c)])}
}
